...
A digest can be run on demand when the person asks, or on a schedule. The highest-value pattern is a scheduled task that delivers a morning summary without the person having to remember to ask, because changelog awareness is prevention work that busy people do not do proactively. If the person wants recurring digests, support setting that up as a scheduled task running this digest for their tracked vendors. Note honestly that a scheduled task runs while their Claude app is open and machine is awake, so timing is best-effort rather than guaranteed-clock. The digest logic below is identical whether run on demand or on schedule.
### Step 1: Load context
...
- Unattended scheduled delivery requires a scheduled task plus, for email-triggered vendors, a connected inbox. Interactive use requires neither.
- The registry ships with one real, complete example entry (Zoom) so the format is self-documenting and the feedless case is demonstrated. Everything else is added through onboarding.
